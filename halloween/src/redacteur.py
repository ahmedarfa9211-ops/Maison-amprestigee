...
def construire_prompt(
    ...
    lexique = config.get("theme", {}).get(
        "champ_lexical",
        "costume, déguisement, accessoire, perruque, masque, maquillage, taille, cape, Halloween, soirée",
    )
...
3. CONCRET : donne des conseils précis et actionnables. Comment prendre ses mesures et lire un guide des tailles, quelle matière tient chaud le soir du 31 octobre, comment poser un maquillage qui tient, quel accessoire transforme un costume, quelles règles de sécurité pour un enfant (visibilité la nuit, inflammabilité, champ de vision, lentilles marquées CE). Ce genre d'information est ce qui fait la valeur de l'article.
4. INTERDIT ABSOLU : n'écris jamais de prix, jamais de montant en euros, jamais d'URL, jamais de lien, jamais de note d'étoiles Amazon, jamais de nom de marque réelle ni de personnage sous licence (pas de nom de film, de super-héros de marque déposée, etc.). Parle des produits par leur « nom exact » donné ci-dessus, orthographié à l'identique — le système transforme ces mentions en liens.
