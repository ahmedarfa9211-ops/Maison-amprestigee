...
class Calendrier:
    ...
    def __init__(self, sujets: dict[str, Any], produits: list[dict[str, Any]]):
        ...
        # Nom générique de la niche, injecté dans les sujets combinés
        # (« déguisement sorcière », « déguisement pour femme »…).
        self.theme_mot = sujets.get("theme_mot", "produit")
    ...
    def _sujets_combines(self, annee: int) -> list[dict[str, Any]]:
        ...
        for genre, cible in cibles:
            for format_ in axes["formats"]:
                # « déguisement sorcière », « déguisement pour femme »… : le
                # nom générique de la niche vient de theme_mot (config data).
                libelle = f"{self.theme_mot} {cible['cle']}"
                mot_cle = f"{self.theme_mot} {cible['cle']}"
    ...
    def produits_pour(self, sujet: dict[str, Any], nombre: int = 6) -> list[dict[str, Any]]:
        ...
        # Un accessoire (maquillage, perruque, masque, cape…) ne doit jamais
        # ouvrir un comparatif de costumes : il ne concourt à armes égales que
        # si le sujet porte explicitement sur ce type de produit, ou sur un
        # déguisement de dernière minute (où cape et masque font le costume).
        SUJETS_ACCESSOIRES = {
            "accessoire", "maquillage", "fx", "faux-sang", "perruque", "masque",
            "cape", "chapeau", "lentilles", "derniere-minute", "diy",
        }
